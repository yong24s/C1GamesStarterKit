...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def build_defences(self, game_state):
        ...
        """
        destructor_locations = [[3, 13], [24, 13], [10, 11], [17, 11]]
    ...
    def deploy_attackers(self, game_state):
        ...
        """
        if game_state.can_spawn(PING, [26, 12], 3):
            game_state.attempt_spawn(PING, [26, 12], 3)


        
        if game_state.can_spawn(EMP, [1, 12]):
            game_state.attempt_spawn(EMP, [1, 12])
        if game_state.can_spawn(PING, [1, 12], 3):
            game_state.attempt_spawn(PING, [1, 12], 3)
